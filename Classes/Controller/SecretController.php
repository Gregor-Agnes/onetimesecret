...
use Zwo3\Onetimesecret\Domain\Repository\SecretRepository;
...
class SecretController extends ActionController
{
    ...
    public function createUnsubscribeMailAction(?string $email = null)
    {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            if ($existing = $this->secretRepository->findOneByEmail($email, false)) {
                ...
                try {
                    ...
                    $this->secretRepository->update($existing);
    ...
    public function createConfirmationAction(Subscription $subscription)
    {
        ...
        // already subscribed
        if ($existing = $this->secretRepository->findOneByEmail($subscription->getEmail(), false)) {
            // Abmelden Mail versenden
            /** @var Subscription $existing */
            ...
            $subscription->setPid($this->secretRepository->createQuery()
                ->getQuerySettings()
                ->getStoragePageIds()[0]);

            $this->secretRepository->add($subscription);
    ...
    public function unsubscribeAction(?int $uid = null, ?string $subscriptionHash = null)
    {
        ...
        $subscription = $this->secretRepository->findByUid($uid, false);

        $success = false;
        if ($subscription) {
            if ($subscriptionHash == $subscription->getSubscriptionHash()) {
                $this->secretRepository->remove($subscription);
                $success = true;
            } else {
                // increasing sleeptimer
                $subscription = $this->setSleep($subscription, 300, 2);
                $this->secretRepository->update($subscription);
